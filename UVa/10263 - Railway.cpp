...
double print(complex<double> a, complex<double> b, complex<double> c)      //distance from point p2 to segment p0-p1
{
	cout.precision(4);
	double d1, d2;
	complex<double> v1 = b - a, v2 = c - a;
	if ((d1 = dotP(v1, v2)) <= 0) { cout <<fixed<< a.real() << endl << a.imag() << endl; return 0; }
	if ((d2 = dotP(v1, v1)) <= d1) {
		cout <<fixed<< b.real() << endl << b.imag() << endl; return 0;
	}
	double t = d1 / d2;
	cout <<fixed<< (a + v1 * t).real() << endl << (a + v1 * t).imag() << endl;
	return length((c - (a + v1 * t)));
}
...
int main()
{
	...
	while (cin >> xm >> ym)
	{
		...
		for (int i = 0; i < n - 1; i++)
		{
			if (dcmp(distToSegment(v[i], v[i + 1], point(xm, ym)), dist) == -1)
			{
				dist = distToSegment(v[i], v[i + 1], point(xm, ym));
				seg = i;
			}
		}
		print(v[seg], v[seg + 1], point(xm, ym));
	}
	return 0;
}