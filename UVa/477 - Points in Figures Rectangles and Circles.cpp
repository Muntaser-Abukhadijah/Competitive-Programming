...
bool pointInsideRec(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, double px, double py)
{
	...
	double A4 = area(px, py, x4, y4, x1, y1);
	if (A == A1 + A2 + A3 + A4)// this cmp not true because it is not for double.
		return 1;
	return 0;
}
bool pointInsideRec2(double x1, double y1, double x2, double y2, double px, double py)
{
	if (dcmp(px, x1) == 1 && dcmp(x2, px) == 1 && dcmp(py, y2) == 1 && dcmp(y1, py) == 1)
		return 1;
	return 0;
...
struct fig
{
	...
	fig()
	{
		x1 = 0, y1 = 0, x2 = 0, y2 = 0, x3 = 0, y3 = 0, x4 = 0, y4 = 0, r = 0;
	}
};
double px, py;
int main()
{
	...
	fig o;
	vector<fig>figure;
	while (scanf(" %c", &o.typ) && o.typ != '*')
	{
		if (o.typ == 'r')
		{
			scanf("%lf%lf%lf%lf", &o.x1, &o.y1, &o.x2, &o.y2);
			o.x3 = o.x2, o.y3 = o.y1;
			o.x4 = o.x1, o.y4 = o.y2;
		}
		else
			scanf("%lf%lf%lf", &o.x1, &o.y1, &o.r);
		figure.push_back(o);
	}
	int cnt = 0;
	while (scanf("%lf%lf", &px, &py))
	{
		...
		for (int i = 0; i < figure.size(); i++)
		{
			if (figure[i].typ == 'r')
			{
				//		bool isIn = pointInsideRec(figure[i].x1, figure[i].y1, figure[i].x2, figure[i].y2, figure[i].x3, figure[i].y3, figure[i].x4, figure[i].y4, px, py);
				bool isIn = pointInsideRec2(figure[i].x1, figure[i].y1, figure[i].x2, figure[i].y2, px, py);
				if (isIn)
				{
					ok = false;
			...
			{
				if (dcmp(length(((complex<double>(px, py)) - (complex<double>(figure[i].x1, figure[i].y1)))), figure[i].r) == -1)
				{
					ok = false;
					printf("Point %d is contained in figure %d\n", cnt, i + 1);
				}
